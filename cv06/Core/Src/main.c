...
int main(void) {
	...
	while (1) {
		// converting and reading from DS18B20
		OWConvertAll();
		//HAL_Delay(CONVERT_T_DELAY);
		OWReadTemperature(&temp_18b20);

		// converting and reading from DS18B20
		temp_data = ntc_data[HAL_ADC_GetValue(&hadc)];
		//stavovy automat

		static enum {
			SHOW_NTC, SHOW_DS18B20
		} state = SHOW_DS18B20;
		...
		case SHOW_DS18B20:
			...
			break;
		case SHOW_NTC:
			// writitng on display NTC
			sct_value((temp_data), 0, 2);
